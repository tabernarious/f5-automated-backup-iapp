...
sftp_function()
{
	...
	echo put BACKUPDIRECTORY/${fname_noext}BACKUPFILENAMEEXTENSION_WITHDOT* | sftp -b- -i /var/tmp/TMSHAPPNAME_sftp.key SFTPCIPHER SFTPSTRICTHOSTKEYCHECKING \${username}@\${server}:./\${directory} 2>> /var/tmp/scriptd.out
	sftp_result=\$?
...
==========
SFTP (in iApp template; looks like debugging remnants)
==========
